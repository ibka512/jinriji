...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-0.9.0-667b68" alt="版本 0.9.0" />
</p>
...
| 整理笔记 | 单层笔记本、紧凑列表、每日／课堂／读书模板、笔记链接与关联记录 |
...
- **安静排版**：“宋”切换衬线正文，“行距”切换宽松行距，“专注”收起分类区域并扩大稿纸。手机格式栏可左右滑动，工具栏随正文滚动保持可用。
...
验收脚本覆盖记录、课程、整理、页面式写作、笔记本与模板、跨篇链接、选段待办、图片表格、备份、离线及多标签页冲突，也检查多端尺寸、深色模式、减少动态与大字号。写作专项包含模拟中文组合输入、真实 IndexedDB 锁下连续输入、保存失败恢复及 1 万／5 万／20 万字符边界。截图输出到 `test-results/screenshots-v0.9/`。这些结果不等同于真实 iPhone / Safari 或真实输入法验证。
