...
function createCourseCard(course: Course): HTMLElement {
  ...
  card.innerHTML = `<span class="course-card__mark">${safeHTML(course.name).slice(0, 1)}</span><span class="eyebrow">自建课程</span><h2>${safeHTML(course.name)}</h2><p>${firstMeeting}</p><footer><span>课程记录</span><span>可继续关联待办</span></footer>`;
  return card;
}
